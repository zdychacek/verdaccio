...
function HTPasswd(config, stuff) {
  ...
  // we need to approve user registration first
  self._approval_needed = config.approval_needed || false
...
HTPasswd.prototype.authenticate = function (user, password, cb) {
  ...
  self._reload(function (err) {
    ...
    // if registration approval is needed and user is not approved already
    if (self._approval_needed && !self._users[user].is_approved) return cb(null, false)
    if (!utils.verify_password(user, password, self._users[user].hash)) return cb(null, false)
...
HTPasswd.prototype.adduser = function (user, password, real_cb) {
  ...
  utils.lock_and_read(self._path, function (err, res) {
    ...
    try {
      body = utils.add_user_to_htpasswd(body, user, password, !self._approval_needed)
    } catch (err) {
      return cb(err)
    }
    fs.writeFile(self._path, body, function (err) {
      ...
      self._reload(function () {
        if (self._approval_needed) {
          console.log('Your registration needs to be approved first. Please send an RT.')
        }
