  ...
  function allow_action(action) {
    return function(user, package, cb) {
      ...
      if (user.name) {
        ...
        cb( Error[403]('Unregistered/not approved users are not allowed to ' + action + ' package ' + package.name + '. Maybe your registration has not been approved yet. (Did you send an RT?).') )
      }
    }
  }
